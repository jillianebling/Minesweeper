...
public class MinesweeperConfig implements Configuration{
    ...
    /**
     * @param toCopy - instance of MinesweeperConfig
     * @param loc - location of most recent move
     */
    private MinesweeperConfig(MinesweeperConfig toCopy, Location loc){
        List <Location> copyCovered = new ArrayList<>();
        List <Location> copyUncovered = new ArrayList<>();

        for (Location l: toCopy.covered) {
            copyCovered.add(l);
        }
        for (Location l: toCopy.uncovered) {
            copyUncovered.add(l);
        }

        this.covered = copyCovered;
        this.uncovered = copyUncovered;
    ...
    public MinesweeperConfig deepCopy (Location loc) {
        return new MinesweeperConfig(this, loc);
    }
    ...
    /**
     * 
     * @param game - instance of GameState
    ...
    /**
     * returns a copy of each possible next move
     */
    @Override
    public Collection<Configuration> getSuccessors() {
        ...
        for (Location loc: uncovered) {
            copyUncovered.add(loc);
        }

        for (Location c: covered) {
            MinesweeperConfig m = deepCopy(c);
            list.add(m);
        }
    ...
    /**
     * returns true if the move given is valid
     */
    @Override
    public boolean isValid() {
        if (locations.get(uncovered.get(uncovered.size()-1)) == 'm')
            return false;
        return true;
    }

    /**
     * returns true if GameState is won
     */
    @Override
